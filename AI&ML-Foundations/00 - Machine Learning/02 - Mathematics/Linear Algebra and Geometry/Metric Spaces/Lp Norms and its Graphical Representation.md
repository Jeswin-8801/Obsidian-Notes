...
$$\large \| \mathbf{x} \|_p = \left( \sum_{i=1}^n |x_i|^p \right)^{1/p} $$
...
> Refer: [[Lp Norms and its Graphical Representation#Geometry of the $L_1$ Unit Ball|Geometry of the L1 Norm]]

> [!NOTE] Note
> - Used in Lasso regression to induce sparsity.
> - The corresponding definition for distance [[Proximity Metrics#Manhattan Distance]]
> - **Non-Uniqueness of Geodesics:** Unlike the L2​ norm, where the shortest path between two points is a unique straight line, there are often **infinitely many shortest paths** between two points under the L1​ metric.
> - Difference between a norm and distance metric: [[Distances vs Norms]]

### The Euclidean Norm ($L_2$ Norm)

In an $n$-dimensional vector space $\mathbb{R}^n$, the $L_2$ norm represents the standard "as-the-crow-flies" distance from the origin. It is defined as the square root of the sum of the squares of its components:

$$\|\mathbf{x}\|_2 = \sqrt{\sum_{i=1}^n x_i^2}$$

> **Geometry:** The set of points where $\|\mathbf{x}\|_2 = 1$ forms a unit **hypersphere**.
> [[#3. $p=2$ (Euclidean Norm)|Geometry of the Euclidean Norm visualized]]
- **Regression:** It is the basis for **Ridge regression** ($L_2$ regularization). Unlike $L_1$, it penalizes larger errors more heavily but rarely forces coefficients to exactly zero.
- **Invariance:** It is **rotationally invariant**, meaning the length of the vector does not change if you rotate the coordinate system.

### The Chebyshev Norm ($L_\infty$ Norm)

The $L_\infty$ norm (also known as the **maximum norm**) focuses strictly on the single most significant dimension of the vector. It is defined as the maximum absolute value among its components:

$$\|\mathbf{x}\|_\infty = \max_{i} (|x_1|, |x_2|, \dots, |x_n|)$$

> **Geometry:** The unit "ball" for the $L_\infty$ norm in 2D is a **square** aligned with the axes, where every point on the boundary has at least one coordinate equal to 1 or -1.
> [[#4. $p=10$ (Approaching $L_ infty$)|Geometry of the Chebyshev Norm visualized]]
...
### 2. $p=1$ ([[Manhattan Distance and Norm#The Manhattan Norm ($L_1$ Norm)|Manhattan]] / Taxicab Norm)
